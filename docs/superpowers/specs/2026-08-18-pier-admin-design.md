...
- 状态：已评审，待实现
- 子项目：第四期（管控后台）
- 前置：`2026-08-16-pier-terminal-design.md`（终端本体设计，第 13 节把管控后台列为后续子项目）
...
| 配置节 `pier.link.employees` / `pier.link.front` | 删除（下沉到数据库） |
...
- 算法 **AES-256-GCM**，密钥 32 字节以 Base64 存于配置项 `pier.crypto.key`，**由部署机环境变量注入**，不进库、不进镜像、不进 git。
...
- `server` 新增环境变量：`SPRING_DATASOURCE_URL` / `_USERNAME` / `_PASSWORD`（外置 MySQL）、`PIER_CRYPTO_KEY`。口令与密钥走部署机上的 `.env`（gitignore），compose 里只写 `${VAR}` 引用。
...
4. 从 `application-prod.yaml` 删除 `pier.link.employees` 与 `pier.link.front`。
