...
| # | 区块 | 底色 | 内容 |
|---|---|---|---|
| 1 | Header | 玻璃白 | 词标 + Lane｜线路 · 终端 · 常见问题｜下载按钮，sticky |
| 2 | Hero | 白 + Mint 光晕 | 主标 + 副标 + 双 CTA + 版本药丸；下方产品窗口示意 |
| 3 | 专属线路 | Cloud | 三节点图讲解 + 三条要点 |
| 4 | 状态一目了然 | 白 | 四张状态卡 |
...
**区块 4「状态一目了然」** 是本次重做的核心新增。它取桌面端 `src/link.ts` 里四种典型处境，
把它们**转述成白话**摆在页面上：

| 对应处境 | 页面上的说法 | tone | 重试措辞 |
|---|---|---|---|
| `NOT_PROVISIONED` | 还没给你分配线路 | muted | 重新检查 |
| `EGRESS_UNREACHABLE` | 线路连不上 | danger | 重新连接 |
| `EGRESS_MISMATCH` | 出口对不上 | warn | 重新连接 |
| `ACTIVE` | 一切正常 | ok | 无 |

叙事重点是「**出了问题它会告诉你是哪儿的问题**」——不是甩一句「连接失败」让用户自己查半天，
而是分清「该等一下」和「该你动手」。这一点在产品里是真的（四种处境各有各的文案与重试入口），
官网只是把它讲成用户听得懂的话。

> ⚠️ 这四张卡是桌面端处境的**白话转述，不是逐字镜像**（逐字搬过来会带上「配置已就位」
> 「出口校验不通过」这类术语，落地页读起来像故障手册）。语义要跟桌面端保持一致，
> 桌面端调整处境划分时官网要跟着改；但用词不必逐字相同。
>
> ⚠️ **术语差异**：官网一律说「线路」，桌面端 UI 里说的是「链路」。官网选前者是为了更口语；
> 这个差异是有意的，但如果哪天要统一，改的应该是桌面端。
