...
```bash
WORK=/tmp/lane-verify && mkdir -p "$WORK"
...
写入 `/tmp/lane-verify/config.yaml`，把 `<...>` 换成上一步抄到的真实字段：
...
```bash
cd /tmp/lane-verify && ./mihomo -d . -f config.yaml &
sleep 3
...
```bash
export CLEAN=/tmp/lane-verify/home && mkdir -p "$CLEAN"
HOME="$CLEAN" \
...
```bash
tail -f /tmp/lane-verify/*.log 2>/dev/null || echo "日志在 mihomo 前台输出中查看"
```
...
```bash
pkill -f "lane-verify/mihomo" ; rm -rf /tmp/lane-verify
...
预期：第一个测试 PASS；第二个在未提供 `LANE_TEST_LINK` 时跳过。

- [ ] **Step 3: 用真实链路跑一次第二个测试**

把 Task 1 用过的两跳节点写成 JSON（字段对应 `LinkConfig`）存到 `/tmp/lane-link.json`，然后：

```bash
MIHOMO_BIN=/tmp/mihomo-test LANE_TEST_LINK=/tmp/lane-link.json mise run test-desktop
```
...
fn local_link_path() -> PathBuf {
    std::env::var("LANE_LINK_FILE")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("/tmp/lane-link.json"))
}
...
准备好 `/tmp/lane-link.json`（含真实两跳节点与 Claude 凭据），然后：

```bash
MIHOMO_BIN=/tmp/mihomo-test LANE_LINK_FILE=/tmp/lane-link.json mise run run-desktop
```
...
5. 手工把 `/tmp/lane-link.json` 里的 `expected_egress_ips` 改成一个错误 IP 重启 App，确认状态条变红且**无法打开终端**

- [ ] **Step 12: 清理并提交**

```bash
rm -f /tmp/lane-link.json   # 含真实凭据，不入库
git add -A
