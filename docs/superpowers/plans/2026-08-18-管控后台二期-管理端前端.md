...
**Spec:** `docs/superpowers/specs/2026-08-18-mintpop-admin-design.md`（第 8、9 节是本计划的正文来源）
...
- Produces:
  ...
  - pnpm 包名 `@mintpop/admin`（`install-admin` 与 Dockerfile 都按这个名字过滤）
...
run = "pnpm install --filter @mintpop/admin ${usage_frozen:+--frozen-lockfile}"
```
...
run = "docker build -f apps/admin/Dockerfile -t mintpop-admin:local ."
```
...
**Interfaces:**
- Consumes: Task 1 的 `install-admin` / `build-admin` / `image-admin` task、pnpm 包名 `@mintpop/admin`
- Produces: 本地镜像 `mintpop-admin:local`，站点根目录期望被挂入一个 `/config.json`
...
Expected: 构建成功，最终镜像基于 nginx alpine（`docker images mintpop-admin:local` 体积在几十 MB 量级，不含 node）。

- [ ] **Step 5: 起一个容器验 nginx 行为**

```bash
docker run --rm -d --name mintpop-admin-试跑 \
...
- Produces: tag 前缀 `admin-v*`、镜像 `ghcr.io/<owner>/mintpop-admin`、mise task `release-admin`
...
env:
  IMAGE: ghcr.io/${{ github.repository_owner }}/mintpop-admin
  ...
  github-release:
    ...
    steps:
      ...
      - name: 发布 Release
        ...
        with:
          name: Mintpop admin v${{ steps.prev.outputs.version }}
          body_path: notes.md
```
...
```yaml
  admin:
    image: ghcr.io/yuebai-blast/mintpop-admin:${ADMIN_TAG:-latest}
    container_name: mintpop-admin
...
```bash
env MYSQL_URL=jdbc:mysql://127.0.0.1:3306/mintpop \
    MYSQL_USERNAME=x MYSQL_PASSWORD=x MINTPOP_CRYPTO_KEY=x \
...
Expected: `Release Admin` 流水线跑通——质量门禁 → 推 `ghcr.io/<owner>/mintpop-admin:0.1.0` 与 `:latest` → 建出标题为 `Mintpop admin v0.1.0` 的 GitHub Release，正文顶部是 tag 注释、下面是按类型过滤的变更日志。
...
- [ ] 部署机上 `mise run up` 后 `mintpop-server` 与 `mintpop-admin` 两个容器都是 `healthy`（需要已发布的镜像与可达的 MySQL，只能在部署机上验）
...
| 2 | `pnpm install --filter @mintpop/admin --frozen-lockfile` 在只有部分 `package.json` 的 Docker 上下文里是否成立 | Task 6 Step 4 | 已在本地工作区实测：缺少其它成员的 `package.json`、甚至没有根 `package.json`，该命令仍只装被选中的包 | 退成把 `apps/desktop/package.json` 也 COPY 进上下文并跑不带 `--filter` 的安装（多装一份桌面端依赖，只影响构建阶段体积） |
