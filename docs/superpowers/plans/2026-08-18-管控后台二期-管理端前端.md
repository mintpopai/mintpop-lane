...
- [ ] **Step 7: 本地用 compose 验一次两个服务并存**

在 `deploy/` 下准备好 `.env`、`application-prod.yaml` 与 `admin-config.json`（本地验证可用假值），然后：

Run: `mise run up && docker compose -f deploy/docker-compose.yml ps`
Expected: `mintpop-server` 与 `mintpop-admin` 两个容器都是 `healthy`；`curl -I 127.0.0.1:8082/` 返回 200。验完 `mise run down`。
