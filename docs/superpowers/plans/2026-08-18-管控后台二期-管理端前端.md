...
**Tech Stack:** Node 26.7.0 / pnpm 11.22.0（版本钉在根 `mise.toml`）/ Vue 3.5.41 / TypeScript 6.0.3 / Vite 8.2.1 / Element Plus 2.14.4 / Pinia 4.0.3 / vue-router 5.2.0 / @logto/browser 3.0.13 / Vitest 4.1.11 + jsdom 30.0.1 + @vue/test-utils 2.4.11 / ESLint 10.8.1 + eslint-plugin-vue 10.10.0 + @vue/eslint-config-typescript 14.9.0 / nginx 1.30-alpine。
...
Run: `mise run test-admin`
Expected: PASS，3 个 describe 共 7 个用例全绿。
...
- Produces:
  ...
  - `applyProtocol(form: NodeFormModel, protocol: NodeProtocol): NodeFormModel`——切协议时重置敏感键、保留已填的透传键
