...
public class AdminSubscriptionServiceImpl implements AdminSubscriptionService {
    ...
    public Long create(Long userId, SubscriptionCreateRequest request) {
        ...
        checkEnterpriseAccepts(request.getEnterpriseId(), plan.getAgentType());
    ...
    public void update(Long id, SubscriptionUpdateRequest request) {
        ...
        checkEnterpriseAccepts(request.getEnterpriseId(), s.getAgentType());
        s.setEnterpriseId(request.getEnterpriseId());
    ...
    /**
     * 归属企业的把关：企业得存在、在启用中，且支持这条订阅的 agent 类型。
     * enterpriseId 为空即个人订阅，不做任何检查。
     */
    private void checkEnterpriseAccepts(Long enterpriseId, AgentType agentType) {
