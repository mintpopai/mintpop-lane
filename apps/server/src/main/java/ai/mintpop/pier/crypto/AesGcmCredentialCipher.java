...
import ai.mintpop.pier.config.CryptoProperties;
...
public class AesGcmCredentialCipher implements CredentialCipher {
    ...
    public AesGcmCredentialCipher(CryptoProperties properties) {
        ...
        try {
            ...
            throw new IllegalStateException("pier.crypto.key 不是合法的 Base64", e);
        }
        if (key.length != KEY_LENGTH) {
            throw new IllegalStateException(
                    "pier.crypto.key 解码后必须是 32 字节，实际为 " + key.length + " 字节");
        }
